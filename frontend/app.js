...
function leafRow(d) {
  ...
  return `<tr class="leaf" data-i="${d._i}">
   ...
   <td class="promo"><input class="inp" data-ch="web" value="${d.promo.web}"></td>${ckc(d.promo.web, d.ny, "web")}${gmc(d.promo.web, d.cogs, "web")}
   <td class="promo"><input class="inp" data-ch="ecom" value="${d.promo.ecom}"></td>${ckc(d.promo.ecom, d.ny, "ecom")}${gmc(d.promo.ecom, d.cogs, "ecom")}<td class="promo"><input class="inp" data-cp="1" value="${rowcp}" title="Phí sàn % — mặc định = CP Sàn+QC"></td><td class="gm ${after === null ? "" : after >= 0 ? "pos" : "neg"}" data-gm="ecomAfter">${pct(after)}</td>
   <td class="promo"><input class="inp" data-ch="dl1" value="${d.promo.dl1}"></td>${ckc(d.promo.dl1, d.ny, "dl1")}${gmc(d.promo.dl1, d.cogs, "dl1")}
   <td class="promo"><input class="inp" data-ch="dl2" value="${d.promo.dl2}"></td>${ckc(d.promo.dl2, d.ny, "dl2")}${gmc(d.promo.dl2, d.cogs, "dl2")}
...
$("body").addEventListener("input", (e) => {
  ...
  if (t.dataset.note !== undefined) { d.note = t.value; return; }
  if (t.dataset.cp !== undefined) { d.cp = parseFloat(t.value); if (isNaN(d.cp)) d.cp = 0; recomputeEcomAfter(tr, d); return; }
  const chn = t.dataset.ch; d.promo[chn] = parseFloat(t.value) || 0;
...
$("refresh").addEventListener("click", () => { if (confirm("Refresh sẽ tải lại số liệu và XÓA giá CTKM đã nhập. Tiếp tục?")) load(); });
